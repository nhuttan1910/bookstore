from app.models import (TheLoai, Sach, TacGia, NhaXuatBan, Sach_TheLoai, Sach_TacGia,
                        KhachHang,HoaDon,ChiTietHD,TaiKhoan,QuiDinh,NhanVien)
...
def load_info_nv(user_id):
    info_nv = NhanVien.query

    info_nv = info_nv.filter(NhanVien.tk_id == user_id)
...
def load_hoadon_nv(user_id):
    hd_nv = HoaDon.query

    hd_nv = hd_nv.filter(HoaDon.nv_id == user_id)
