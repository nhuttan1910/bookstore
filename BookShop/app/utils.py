...
import json, os
from app.models import TaiKhoan, KhachHang, ForeignKey, Sach,TheLoai, Sach_TheLoai, VaiTro, ChiTietHD, HoaDon, QuiDinh, HoaDonNhap, ChiTietHDN,NhaXuatBan,NhanVien
import hashlib
...
def cre_hd(viewtt):
    info = NhanVien.query
    info = info.filter(NhanVien.tk_id == current_user.id)

    hd = HoaDon(nv_id = info[0].id ,thanhtoan=1)
    db.session.add(hd)
    db.session.commit()
